...
//command line reuires an endpoint ticket when config is empty.
...
pub struct Config {
    topic: String,
    mother_ship: Option<Vec<EndpointId>>,
    secret: SecretKey,
}

impl Config {
    const FILE_NAME: &str = "settings.toml";
    fn load() -> Result<Config, AnyError> {
...
async fn main() -> Result<()> {
    ...
    // endpoint , this needs to timeout
    let _ = endpoint.online().await;
    println!("{:#?}", &endpoint.id());

    if let Some(mother_ship) = config.mother_ship {
        let my_topic = make_topic(config.topic.as_str());
        let goss = gossip.subscribe_and_join(my_topic, mother_ship).await?;
